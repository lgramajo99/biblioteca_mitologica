...
import { addFile, listUl, users, chartPie, squarePoll, calendar, doorOpen, menu, arrowCollapse } from "../../../assets/index";
...
function DashboardMenu() {
    ...
    return (
        <nav className={`fixed top-0 left-0 h-full px-2 bg-purple-700 dark:text-slate-300 dark:bg-darkPrimary shadow-md transition-all duration-500 ${displayedMenu ? "w-80" : "w-20"} overflow-y-auto`} >
            ...
            <button type="button"
                ...
                <img src={arrowCollapse} className={`filter dark:invert`} alt="menu" />
            </button>
            ...
            <ul className="overflow-hidden gap-y-2 flex flex-col">
                ...
                <button type="button"
                    onClick={logout}
                    className={`flex items-center w-full rounded-xl p-1 gap-2 transition duration-300 ease-in-out focus:font-bold focus:outline-none hover:font-bold hover:bg-purple-200 focus:bg-purple-200 dark:text-darkTxt dark:hover:bg-darkSecondary dark:focus:bg-darkSecondary ${displayedMenu ? 'justify-start' : 'justify-center'}`}>
                    <img src={doorOpen} alt='door open' className={`filter dark:invert`} />
